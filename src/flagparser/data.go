...
func initData() {
	data = flagData{
		...
		WaitData:         MinWaitSec,
		WaitName:         "wait",
		WaitUseage:       fmt.Sprintf("Delay starting the service in seconds. The option is a number, the default value is 0. The minimum value is 0 and the maximum value is 60. This option is mainly used to restart the backend service to reserve enough time for the previously running backend service to shut down. %s", strings.ToUpper("Note: Generally, you do not need to set this option.")),
		Useage:           "",
	}
...
func (d *flagData) Wait() uint {
	...
	if d.WaitData > MaxWaitSec {
		return MaxWaitSec
	} else if d.WaitData < MinWaitSec {
		return MinWaitSec
	} else {
		return d.WaitData
	}
