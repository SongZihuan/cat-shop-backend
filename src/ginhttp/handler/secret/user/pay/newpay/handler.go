...
func Handler(c *gin.Context) {
	...
	if query.WupinID <= 0 {
		c.JSON(http.StatusOK, data.NewNotSuccessData(CodeWupinNotFound, "物品未找到", "WupinID必须大于0"))
		return
	}

	if query.Num <= 0 {
		c.JSON(http.StatusOK, data.NewNotSuccessData(CodeNumMustMaxThanZero, "购买数量不对", "购买数量必须大于0"))
		return
	}
	...
	if errors.Is(err, action.ErrNotFound) {
		c.JSON(http.StatusOK, data.NewNotSuccessData(CodeWupinNotFound, "商品未找到"))
		return
	} else if err != nil {
