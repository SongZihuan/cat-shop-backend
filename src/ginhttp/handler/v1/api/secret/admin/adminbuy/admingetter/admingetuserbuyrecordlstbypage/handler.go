package admingetuserbuyrecordlstbypage
...
func Handler(c *gin.Context) {
	...
	res, err := adminaction.AdminGetBuyRecordListByPageByUser(user, query.Page, query.PageSize)
	if err != nil {
		c.JSON(http.StatusOK, data.NewSystemDataBaseError(err))
		return
	}

	maxcount, err := adminaction.AdminGetBuyRecordLCountByPageByUser(user)
	if err != nil {
		c.JSON(http.StatusOK, data.NewSystemDataBaseError(err))
		return
	}
